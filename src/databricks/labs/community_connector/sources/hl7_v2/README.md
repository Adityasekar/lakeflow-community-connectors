...
| CQ | Composite quantity with units | STRING | Both components flattened via `_cq_fields()` / `_cq_schema()` (2 columns): `{prefix}` (CQ.1, NM quantity), `{prefix}_units` (CQ.2.1, CWE units code). Used by `ft1.ndc_qty_and_uom*`. |
| DR | Date/time range | STRING | Both components flattened as two `_ts` columns: `{prefix}_start` (DR.1, DTM) and `{prefix}_end` (DR.2, DTM). Used by `ft1.transaction_date_start` / `ft1.transaction_date_end`. |
